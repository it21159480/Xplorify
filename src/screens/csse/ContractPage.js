...
const ContractPage = ({ navigation }) => {
    ...
    const handleSearch = (text) => {
        ...
        if (text.trim() === '') {
            ...
            const filteredResults = Contract.filter(
                (quotation) =>
                    quotation.name.toLowerCase().includes(text.toLowerCase()) ||
                    quotation.endDate.toLowerCase().includes(text.toLowerCase()) ||
        ...
        return (
            ...
            <View style={[style.topFoodCard, { borderColor: getBorderColor() }]}>
                ...
                <View style={{ borderRadius: 10, flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Pressable style={{ backgroundColor: '#3F3825', marginTop: 10, borderRadius: 10, }}>
                ...
                {searchQuery.trim() ?
                    (
                        <FlatList
                            ...
                            renderItem={({ item }) => <SearchList lists={item} />}
                        />
                    ) :
                    (

                        <ContractCard navigation={navigation} list={Contract} />
